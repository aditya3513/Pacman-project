...
def enhancedFeatureExtractorDigit(datum):
    ...
    1) whenever we call check pixel function we set the visited array as true to represent that 
    we have visited this pixel. [this is an recursive function]

    2) we check if the x and y are within the range i.e 0 and height and width respectively.
    3) we check if we have visited at x,y then we skip it.
    4) we check empty areas >=1 and do this in all directions
    ...
    has_loop=False
    run_dfs_hash={}
    def check_in_pixels(x,y):
            # check if the (x,y) have not been visited previously
        if (x,y) in visited.keys():
            return True
                    # check if the x and y are inside the boundries
        if x<0 or x>=DIGIT_DATUM_WIDTH or y<0 or y>=DIGIT_DATUM_HEIGHT:
            return False

        if datum.getPixel(x,y) >=1: # check if empty spaces is more than 1
            return True

        visited[(x,y)]=True # if none of above conditions mee then set this location as visited.

        # recurse in all directions.

        return check_in_pixels(x-1,y) and check_in_pixels(x+1,y) and check_in_pixels(x,y-1) and check_in_pixels(x,y+1)

    for x in range(DIGIT_DATUM_WIDTH): # check for all elements in the width
        for y in range(DIGIT_DATUM_HEIGHT): # check for all elements in the width
            visited={}  # list for checking the visited elements
            if datum.getPixel(x,y)==0:
                if check_in_pixels(x,y)==True: 
                    has_loop=True
                    break

    features["loops"] = has_loop
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    # here i am claculating the distance from food to the pacman
    # and adding each distance inverse as a tuple to the feature list
    successor = state.generateSuccessor(0, action)
    for food in successor.getFood().asList():
        i = "food"+str(food)
        features[i] = 1 / util.manhattanDistance(food, successor.getPacmanPosition())
