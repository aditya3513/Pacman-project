...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
       ...
       --> THIS IS HOW IT WORKS :
            ...
            1) check if the noisydistance is none i.e ghost is captured
            2) if yes then set allpossible to 1, else make its value equal to
              (emission model value at that position) * belief at that position
            3) we do this for all legal positions.
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        ---> HERE IS WHAT IS HAPPENING :
            taking help from above mentioned comments we get newPosDist and we just create a new
            util.counter named as belief_new where we store updated belief value by :
            --> adding belief at new pos with itself and [(self.belief at old pos) * (prob)]
            we set this equal to self.belief for updating overall belief

        """
        "*** YOUR CODE HERE ***"

        newPostDist = util.Counter()
        belief_new = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                belief_new[newPos] += self.beliefs[oldPos] * prob
        self.beliefs = belief_new
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while count < self.numParticles:
            for i in range(0, len(self.legalPositions), 1):
                if count < self.numParticles:
                    ctr = count - (count / len(self.legalPositions)) * len(self.legalPositions)
                    self.particles.append(self.legalPositions[ctr])
                    count += 1
    ...
    def observe(self, observation, gameState):
        ...
        1) we check if noisy distance == 0, all ghost caputred for ghost if yes then set
            self.particles = [self.getJailPosition()] * self.numParticles

        2) if ghost are alive then calculate distance then
        using manhattan distance for each position of pacman and ghost

        3) count for each position the value of emissionmodel and beliefDistribution using allPossible a counter

        4) if nothin in allPossible then again initialize everything to fine out errors

        5) and if there is something in allPossible then all all sample value to self.particles
        ...
        if noisyDistance == None:
            ...
            for position in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    allPossible[position] = emissionModel[trueDistance] * self.getBeliefDistribution()[position]
    ...
    def elapseTime(self, gameState):
        ...
        --> working as explained in the comments
        """
        "*** YOUR CODE HERE ***"

        Particles = self.particles
        self.particles = []
        for oldPos in Particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
    ...
    def getBeliefDistribution(self):
        ...
        for all particles in self.particles, count beliefdist by adding previous value with
        number of partciles and then we normalize all these values so that they add up to 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        particle_count = 0  # now we calculate all permuations of legalPositions
        permutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(permutations)  # shuffling them

        # now we add all permutation corresponding to each particle into self.particles
    ...
    A) for all ghosts :
        1) check if ghost exist by noisyDistances[i] == None, if yes
            then for all particles getParticleWithGhostInJail and update the self.particles for each particle
        2)if not then  :
            calculate distance between ghost and pacman by manhattan distance and
            then calculate beliefDistribution[particle] *= emissionModels[i][trueDistance]
    B) if self.getBeliefDistribution() is empty then initialize particles

    C) else normalize beliefDistribution and sample each beliefDistribution
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                for counter in range(self.numParticles):
                    self.particles[counter] = self.getParticleWithGhostInJail(self.particles[counter], i)
                beliefDistribution = self.getBeliefDistribution()
            else:
                for particle in beliefDistribution:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    beliefDistribution[particle] *= emissionModels[i][trueDistance]
