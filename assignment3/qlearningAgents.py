...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            q_star = self.getQValue(state, action)
            if q_star > max :
                max = q_star
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in feature_vector:
            q_star += self.weights[feature] * feature_vector[feature]
    ...
    def update(self, state, action, nextState, reward):
        ...
        Q_new = reward + (self.discount * self.getValue(nextState))
        Q_old = self.getQValue(state, action)

        feature_vector = self.featExtractor.getFeatures(state, action)

        for feature in feature_vector:
            self.weights[feature] += self.alpha * (Q_new - Q_old) * feature_vector[feature]
